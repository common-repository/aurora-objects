<?php
class AuroraObjectsForWordpressS3PluginPublic {
    var $options;
    var $s3;
	var $meta;

	function AuroraObjectsForWordpressS3PluginPublic() {
		$this->options = array();
		if (file_exists(dirname(__FILE__).'/config.php')) {
			require_once(dirname(__FILE__).'/config.php');
			if ($AuroraObjectsForWordpressS3Config) $this->options = $AuroraObjectsForWordpressS3Config;
		}
	...
	function wp_get_attachment_url($url, $postID) {
        ...
        if ($this->options['wp-uploads'] && ($amazon = get_post_meta($postID, 'amazonS3_info', true))) {
	    	...
	    	elseif ( isset($this->options['virtual-host']) && $this->options['virtual-host'] ) {
				...
				$accessDomain = $amazon['bucket'] . '.o.auroraobjects.eu';
	        }
	...
	*/
	function get_secure_attachment_url($postID, $expires = 900, $operation = 'GET') {
		...
		$accessDomain = $this->options['virtual-host'] ? $amazon['bucket'] : $amazon['bucket'].'.o.auroraobjects.eu';
...
function wps3_get_secure_attachment_url($postID, $expires = 900, $operation = 'GET') {
	...
	return $AuroraObjectsForWordpressS3Plugin->get_secure_attachment_url($postID, $expires, $operation);
}